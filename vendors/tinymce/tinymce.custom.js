...
function tinymce_custom_init(options) {
    $(document).ready(function () {
        ...
        for (const key in tinymce_custom_global_options) {
        ...
        opt.plugins = ['code', 'codesample', 'link', 'lists', 'help'];
        ...
        opt.setup = function (editor) {
            this.on('init', function () {
                ...
                const current_title = $('#' + this.id + '_ifr').attr("title");
                const label = $("label[for="+this.id+"]")[0];
                if (label) {
