...
jsu.isInstance = function (obj, type) {
    if (typeof obj == 'object') {
        const matching = obj.constructor.toString().match(new RegExp(type, 'i'));
...
// Get user agent and platform related functions
...
jsu.focusLastDescendant = function (element) {
    for (let i = element.childNodes.length - 1; i >= 0; i--) {
        const child = element.childNodes[i];
        if (jsu.attempt_focus(child) || jsu.focusLastDescendant(child)) {
            return true;
        }
    }
    return false;
